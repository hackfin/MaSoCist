/* Startup code for ZPUng main code
 * This is the experimental code for external bootstrapping
 * from cached ROM area.
 *
 * Sequence:
 * 1) Initialize cache in _config routine
 * 2) Then jump into bootstrap at fixed virtual ROM address
 *    This copies main code into the instruction RAM above the ucode area
 * 3) Jump into main
 *
 */
	.file	"crtmain.s"
...
_g_mmrptr:
	...
	; The Cache IRQ handler MUST live in the ucode area, otherwise
	; the IRQ logic will rearm too early on the last POPPC and get caught
        .balign 32,0
	.globl cache_exc_handler
	cache_exc_handler:
		; Fix up return PC to resume previous insn:
		im -1
		add
		; We don't do an explicit call, the poppc will now jump into
		; the cache handler.
		im cache_handler
		poppc
