...
class ELFSection:
	...
	def __init__(self, endianness = '>'):
		...
		self.data = None
		self.lma =	None
		self.shdr = endianness + self.Elf32_Shdr
	def fromString(self, s):
		"""get section header from string"""
		(self.sh_name, self.sh_type, self.sh_flags, self.sh_addr,
		 self.sh_offset, self.sh_size, self.sh_link, self.sh_info,
		 self.sh_addralign, self.sh_entsize) = struct.unpack(self.shdr, s)
...
class ELFProgramHeader:
	...
	def __init__(self, endianness = ">"):
		...
		self.data = None
		self.phdr = endianness + self.Elf32_Phdr
	def fromString(self, s):
		"""parse header info from string"""
		(self.p_type, self.p_offset, self.p_vaddr, self.p_paddr,
			self.p_filesz, self.p_memsz, self.p_flags,
			self.p_align) = struct.unpack(self.phdr, s)
...
class ELFObject:
	...
	#encoding
	ELFDATANONE		= chr(0)		#Invalid data encoding
	ELFDATA2LSB		= chr(1)		#See below
	ELFDATA2MSB		= chr(2)		#See below

	def __init__(self, default_endian = '>'):
	...
	def fromFile(self, fileobj):
		...
		default_endian = ">" # Try big endian read
		ehdr = default_endian + self.Elf32_Ehdr

		try:
			rawheader = fileobj.read(struct.calcsize(ehdr))
			h = struct.unpack(ehdr, rawheader)

			e = h[0][self.EI_DATA]
			if e == self.ELFDATA2LSB:
				endian = "<"
				# Read again:
				ehdr = endian + self.Elf32_Ehdr
				h = struct.unpack(ehdr, rawheader)
			elif e == self.ELFDATA2MSB:
				endian = ">"
			else:
		...
		phdr = endian + ELFProgramHeader.Elf32_Phdr
		self.programmheaders = []
		if self.e_phnum:
			#load program headers
			fileobj.seek(self.e_phoff)
			for sectionnum in range(self.e_phnum):
				shdr = (fileobj.read(self.e_phentsize) + '\0'* struct.calcsize(phdr))[0:struct.calcsize(phdr)]
		...
		for sectionnum in range(self.e_shnum):
			elfsection = ELFSection(endian)
			shdr = (fileobj.read(self.e_shentsize) + '\0'* struct.calcsize(elfsection.shdr))[0:struct.calcsize(elfsection.shdr)]
